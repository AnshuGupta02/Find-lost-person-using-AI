...
def case_details(case_id: str):
    query = f"select name, father_name, image, mobile, age, submitted_on from submitted_cases where case_id={case_id}"
    with PostgresConnection() as conn:
        cursor = conn.cursor()
        cursor.execute(query)
        return cursor.fetchall()
...
def user_details(case_id: str):
    query = (
        f"select location, submitted_at, image, submitted_by, mobile from user_submissions where id={case_id}"
    )
    with PostgresConnection() as conn:
        cursor = conn.cursor()
        cursor.execute(query)
